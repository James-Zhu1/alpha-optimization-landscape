...
# Kept separate from REPORT.md on purpose: that document is the frozen study,
# this one is a living audit that changes every time the window grows.
FORWARD_REPORT_PATH = ROOT / "FORWARD_TEST.md"
...
def frozen_expressions(path: Path | None = None) -> pd.DataFrame:
    ...
    """
    path = path if path is not None else OUT_OF_SAMPLE_PATH
    if not path.exists():
...
def evaluate_forward_window(
    *,
    ...
    for _, candidate in frozen_expressions(holdout_path).iterrows():
...
def format_summary(results: pd.DataFrame) -> str:
    """Render the forward result as a short, self-limiting markdown document."""
    lines = [
...
def main() -> None:  # pragma: no cover - thin CLI
    ...
    FORWARD_REPORT_PATH.write_text(summary, encoding="utf-8")
    LOGGER.info("Wrote %d forward evaluations to %s", len(results), FORWARD_RESULTS_PATH)
    LOGGER.info("Wrote %s", FORWARD_REPORT_PATH)
    print(summary)
